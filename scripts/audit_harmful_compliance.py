"""Verify pair-level review status consistency for restricted JBB records."""
...
def main():
    ...
    for pair_id, rows in pairs.items():
        ...
        values = {r.get("reviewer_status") for r in rows}
        if len(values) != 1: failures.append(f"{pair_id}: inconsistent review metadata"); continue
        status = values.pop()
        if status != "approved": failures.append(f"{pair_id}: missing completed review")
