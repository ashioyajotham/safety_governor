...
def main():
    ...
    problems = []
    if set(counts) != CANONICAL_ARCHETYPES: problems.append(f"archetype set mismatch: {sorted(counts)}")
    problems.extend(f"{a}: {counts.get(a, 0)} < {args.minimum_per_archetype}" for a in sorted(CANONICAL_ARCHETYPES) if counts.get(a, 0) < args.minimum_per_archetype)
    ...
    if args.minimum_per_archetype == 30 and len(rows) != 120: problems.append(f"expected exactly 120 frozen rows; found {len(rows)}")
