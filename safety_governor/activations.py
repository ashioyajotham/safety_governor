...
def save_matrix(path: str | Path, values: np.ndarray, *, layer: int, token_mode: str, sample_ids: list[str]) -> None:
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    np.save(path, values)
    path.with_suffix(path.suffix + ".json").write_text(json.dumps({
        "layer": layer, "token_mode": token_mode, "sample_ids": sample_ids,
