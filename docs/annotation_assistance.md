...
The current provider-specific annotation adapter notebook is [`notebooks/providers/gemini/ifeval_annotation_colab.ipynb`](notebooks/providers/gemini/ifeval_annotation_colab.ipynb). It verifies the pinned IFEval source hash and workload composition, writes neutral artifact names, and never marks records approved or commits generated output. Provider provenance stays in the audit trail rather than the experiment inputs, and any future adapter must follow the same output contract.
