...
The deceptive diverse corpus is now approved, split-ready, and frozen into the tracked release input at `datasets/frozen/english_contrastive.jsonl`. The approved-ready materialization remains available under `data/working/deceptive_reasoning/deceptive_diverse_approved_ready.jsonl` for auditability, and the approved motivated-reasoning review queue is recorded separately.
